<?php
function ultimate_blog_layouts_render_blog_grid($attributes){
	...
	if ( $query->have_posts() ) {
		...
		$post_output.= '<section class="ultimate-blog-layouts blog-grid">';

		if(isset($attributes['column'])){
			$column = "blg-col-{$attributes['column']}";
		}

		$post_output.= sprintf('<div class="blog-grid-wrapper %1$s">',
			esc_attr($column)
		);
		...
		while ( $query->have_posts() ) {
			...
			if ( $attributes['excerptLength'] ) {
				$excerpt = wpautop(ultimate_blog_layouts_the_excerpt(absint($attributes['excerptLength'])));
			}
			...
			if($attributes['layouts'] === 'layout-3' ){
				$post_output.= sprintf('<div class="blg-inner-box">%1$s %2$s </div>',
					$title,
					$content );
			}
			if ($attributes['showImage'] && get_the_post_thumbnail()) {
				$post_output.= sprintf('<div class="blg-image-wrapper"><a href="%1$s">%2$s</a></div>', 
					esc_url(get_permalink()), 
			...
			if($attributes['layouts'] === 'layout-1' ){
				$post_output.= sprintf('<div class="blg-inner-box">%1$s %2$s</div>', 
					$title, 
					$content );
			}
		...
		$post_output.= sprintf('<p>%1$s</p>', 
			esc_html__('No Posts','ultimate-blog-layouts'));
	}
