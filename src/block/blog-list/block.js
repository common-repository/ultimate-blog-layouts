 ...
 */
 registerBlockType( 'blg/blog-list', {
	// Block name. Block names must be string that contains a namespace prefix. Example: my-plugin/my-custom-block.
	title: __( 'Blog List','ultimate-blog-layouts'), // Block title.
	description: __( 'Display Posts in List','ultimate-blog-layouts'), // Block description.
	...
	keywords: [
	__("Blog List",'ultimate-blog-layouts'),
	__("Blogs",'ultimate-blog-layouts'),
	__("Posts",'ultimate-blog-layouts'),
	__("Ultimate Blog Layouts List",'ultimate-blog-layouts')
	],
	attributes,
	...
	*/
	...
	( function( props ) {
		...
		const { 
			excerptLength, showExcerpt, category, order, orderBy, perPage,
		...
		<section className={ 'ultimate-blog-layouts blog-list'} >
		<div className={'blog-list-wrapper'}> 
		{ props.posts.map((post) =>
			<article className={ 'blog-item'}>
			
			{ (showImage && post.featured_media && layouts !== 'layout-3') ? (
				<div className = {'blg-image-wrapper ' + layouts } >
			...
			{(layouts === 'layout-1' || layouts === 'layout-3') &&
			...
			{showExcerpt && 
				...
				dangerouslySetInnerHTML={{__html: test(post.excerpt.rendered, excerptLength) }}
				/> )
			}
			{showText && 
